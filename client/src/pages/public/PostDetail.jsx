...
export default function PostDetail() {
  ...
  if (!post) return (
    <div className="max-w-4xl mx-auto px-4 py-16 text-center">
      ...
      <Link to="/" className="text-red-700 hover:underline mt-4 block">Kembali ke Beranda</Link>
    </div>
  );
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <article className="lg:col-span-2">
            <nav className="text-sm text-gray-500 mb-4">
              ...
              <Link to={`/categories/${post.category_slug}`} className="hover:text-red-700">{post.category_name}</Link>
              <span className="mx-2">/</span>
              <span className="text-gray-900">{post.title?.substring(0, 50)}</span>
            </nav>

            <span className="inline-block px-4 py-2 bg-red-100 text-red-700 text-xs font-medium rounded mb-4">{post.category_name}</span>
            ...
            <div className="sticky top-24">
              {post.relatedPosts?.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4 pb-2 border-b-2 border-red-700">Berita Terkait</h3>
                      ...
                      <Link key={related.id} to={`/posts/${related.slug}`} className="block group">
                        <div className="flex gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-sm font-semibold text-gray-900 group-hover:text-red-700 line-clamp-2">{related.title}</h4>
